...
type UnsafePluginNames<PluginName extends string = string> = PluginName extends PluginKey ? never : PluginName;
				...
				unsafePluginLoadersByPluginName[unsafePluginCfg.pluginName] =
					async () => {
						...
						if (!pluginLoaderRes.ok) {
							throw new MiniflareCoreError(
								"ERR_RUNTIME_FAILURE",
								JSON.stringify(pluginLoaderRes.error)
							);
						}
...
export class Miniflare {
	...
	constructor(opts: MiniflareOptions) {
		...
		const [sharedOpts, workerOpts, externalPluginLoaders] = validateOptions(opts);
